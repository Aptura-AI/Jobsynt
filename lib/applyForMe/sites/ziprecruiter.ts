/**
 * ZipRecruiter Site Adapter
 * 
 * Isolated adapter for ZipRecruiter job applications.
...
export const ZIPRECRUITER_SITE: SupportedApplySite = 'ZIPRECRUITER';

/**
 * Detect if login is required on ZipRecruiter
 */
export async function detectZipRecruiterLoginRequired(page: Page): Promise<boolean> {
  // ZipRecruiter-specific login detection
  const ziprecruiterLoginSelectors = [
    'a[href*="/login"]',
    'a[href*="/signin"]',
  ...
  for (const selector of ziprecruiterLoginSelectors) {
    try {
...
/**
 * Navigate to ZipRecruiter signup
 */
export async function navigateToZipRecruiterSignup(page: Page): Promise<boolean> {
  try {
    ...
    console.error('[ZipRecruiter] Signup navigation failed:', error);
    return false;
  }
}

/**
 * Apply to job on ZipRecruiter
 */
export async function applyToZipRecruiterJob(params: {
  page: Page;
  jobUrl: string;
  ...
  try {
    ...
    // Check for CAPTCHA (ZipRecruiter often uses CAPTCHA)
    if (await detectCaptcha(page)) {
      const pauseResult = await pauseForIntervention({
        ...
        jobTitle,
        jobCompany,
        site: ZIPRECRUITER_SITE,
      });
      return { success: false, paused: true };
    }

    // Check for login requirement
    if (await detectZipRecruiterLoginRequired(page)) {
      ...
      if (runData) {
        ...
        if (credentials) {
          // Attempt auto-login
          const loginSuccess = await attemptZipRecruiterLogin(page, credentials.email, credentials.password);
          
          if (loginSuccess) {
            // Login successful, continue
            console.log('[ZipRecruiter] Auto-login successful');
          } else {
    ...
    const applyButton = page.locator(
      'button:has-text("Apply"), a:has-text("Apply"), button:has-text("Apply Now"), a:has-text("Apply Now"), button:has-text("Quick Apply")'
    ).first();

    if (!(await applyButton.isVisible({ timeout: 5000 }))) {
      return { success: false, error: 'Apply button not found on ZipRecruiter' };
    }
    ...
    return { success: false, error: error.message || 'ZipRecruiter application failed' };
  }
}

/**
 * Attempt auto-login to ZipRecruiter
 */
async function attemptZipRecruiterLogin(page: Page, email: string, password: string): Promise<boolean> {
  try {
    // Find login form
    ...
    console.error('[ZipRecruiter] Auto-login failed:', error);
    return false;
  }
}
